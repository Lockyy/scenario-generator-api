...
(($, window, document) ->
  ...
  Plugin::next = (effect) ->
    ...
    if effect is "fade" then @_fade()
    if effect is "carousal" then @_carousal() else @_slide()
  ...
  Plugin::goto = (number) ->
    ...
    if typeof number is "number"
      if effect is "fade" then @_fade(number)
      if effect is "carousal" then @_carousal(number) else @_slide(number)
    else if typeof number is "string"
      if number is "first"
        if effect is "fade" then @_fade(0)
        if effect is "carousal" then @_carousal(0) else @_slide(0)
      else if number is "last"
        if effect is "fade" then @_fade(@data.total)
        if effect is "carousal" then @_carousal(@data.total) else @_slide(@data.total)
  ...
  Plugin::_setuptouch = () ->
    ...
    if @options.effect.slide.direction is "up"
  ...
  Plugin::_touchend = (e) ->
    ...
    # Slide has been dragged to the left, goto next slide
      ...
      # Slide has not been dragged far enough, animate back to 0 and reset
        ...
        # Set CSS3 styles
        directionString = if @options.effect.slide.direction is "up" then "translateY" else "translateX"
        slidesControl[0].style[transform] = directionString + "(0px)"
  ...
  Plugin::_touchmove = (e) ->
    ...
    if !@data.animating && !@data.scrolling
      ...
      directionString = if @options.effect.slide.direction is "up" then "translateY" else "translateX"
      slidesControl[0].style[transform] = directionString + "(" + (touches.pageX - @data.touchstartx) + "px)"
  ...
  # @_carousal()
  # This effect only works with four slides, labeled as carousal slides and
  # their positions set initially with the classes 'front', 'back', 'left', and 'right'.
  Plugin::_carousal = (number) ->
    $element = $(@element)
    @data = $.data this
    ...
    if $element.data('animating') != true
      ...
      right = $('.slide.carousal-slide.right')
      front = $('.slide.carousal-slide.front')
      left = $('.slide.carousal-slide.left')

      if @data.direction is "next"
        right.removeClass 'right'
        right.addClass 'front'
  ...
  Plugin::_slide = (number) ->
    ...
    if not @data.animating and number isnt @data.current + 1
      ...
      if @data.vendorPrefix
        ...
        # Set CSS3 styles
        directionString = if @options.effect.slide.direction is "up" then "translateY" else "translateX"
        slidesControl[0].style[transform] = directionString + "(" + direction + "px)"
        ...
        slidesControl.on "transitionend webkitTransitionEnd oTransitionEnd otransitionend MSTransitionEnd", =>
          ...
          # Reset slideshow
          if @options.effect.slide.direction is "up"
            slidesControl.children(":eq(" + next + ")").css top: 0

...
describe 'Collection#owned' do
  before do
    ...
    # Owned by @primary_user.
    # Not owned by @user_2 or @user_3.
    ...
    # Owned by @user_2.
    # Not owned by @primary_user or @user_3.
  ...
  describe 'owned collections' do
    it 'are included for the owner' do
      expect(Collection.all.owned(@owned_collection.user).map(&:id)).to include @owned_collection.id
    end
  end

  describe 'visible collections' do
    it 'are not included for non-owners' do
      expect(Collection.all.owned(@primary_user).map(&:id)).to_not include @visible_collection.id
      expect(Collection.all.owned(@user_3).map(&:id)).to_not include @visible_collection.id
    end
    ...
    describe 'when shared' do
      describe 'as a viewer' do
        ...
        it 'are not owned by that user' do
          expect(Collection.all.owned(@primary_user).map(&:id)).to_not include @visible_collection.id
        end
      end
      ...
      describe 'as an owner' do
        ...
        it 'are owned by that user' do
          expect(Collection.all.owned(@primary_user).map(&:id)).to include @visible_collection.id
        end
      end
    end
  end
  ...
  describe 'hidden collections' do
    describe 'when not shared' do
      it 'are not included' do
        expect(Collection.all.owned(@primary_user).map(&:id)).to_not include @hidden_collection.id
      end
    end
    ...
    describe 'when shared' do
      ...
      describe 'as an owner' do
        ...
        it 'are included when that user is passed in' do
          expect(Collection.all.owned(@primary_user).map(&:id)).to include @hidden_collection.id
        end

        it 'are not included when a different user is passed in' do
          expect(Collection.all.owned(@user_3).map(&:id)).to_not include @hidden_collection.id
        end
      end
    end
  end
  ...
  describe "when running on a user's collections" do
    before do
      @user_2.collections.first.share([{id: @primary_user.id, rank: 2}])
    end

    it 'does not return any collections owned by other users' do
      expect(@user_2.collections.owned(@primary_user).map(&:user).map(&:id).uniq).to eq [@user_2.id]
    end
  end

  describe "when running on a product's collections" do
    it 'only returns collections that contain that product' do
      collections = @product_1.collections.owned(@user_2)
